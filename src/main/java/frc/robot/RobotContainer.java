...
public class RobotContainer {
  ...
  // private final Vision m_rightCamera = new
  // Vision(PhotonConstants.kRightCameraName,
  // PhotonConstants.kRightCameraLocation);
  ...
  // private final AutosCommands m_autosCommands = new AutosCommands(m_robotDrive,
  // m_limelight, m_shooter, m_intake);
  ...
  private final Superstructure m_superstructure = new Superstructure(m_robotDrive, m_leftCamera, m_climber, m_amp,
      m_blinkin, m_driverController, m_operatorController);
  ...
  public RobotContainer() {
    ...
    NamedCommands.registerCommand("intakeFront",
        m_stateMachine.intakeSelectCommand(StateMachine.IntakeState.INTAKE_FRONT));
    NamedCommands.registerCommand("extakeFront",
        m_stateMachine.intakeSelectCommand(StateMachine.IntakeState.EXTAKE_FRONT));
    NamedCommands.registerCommand("shoot", new SequentialCommandGroup(m_superstructure.shoot(), new WaitCommand(0.1),
        new InstantCommand(() -> m_robotDrive.disableRotatingToGoal()), m_superstructure.stopShooter()));
    ...
    // NamedCommands.registerCommand("setupAllianceZone",
    // m_shooter.readyAllianceZone());
    // NamedCommands.registerCommand("setupSlow", new InstantCommand(() ->
    // m_shooter.setFlywheelVelocity(1000)));
    // NamedCommands.registerCommand("setupFast", new InstantCommand(() ->
    // m_shooter.setFlywheelVelocity(8000)));

    // NamedCommands.registerCommand("setDisruptorGyro", new InstantCommand(() ->
    // m_robotDrive.setHeading(143.43)));
    ...
    // NamedCommands.registerCommand("alignToGoal", new RotateToGoal(m_robotDrive,
    // m_leftCamera));
    // NamedCommands.registerCommand("pivot to 50",
    // m_shooter.setPivotAngleCommand(30));
  ...
  private void configureButtonBindings() {
    ...
    // m_driverController.b().whileTrue(new DriveToAmp(m_robotDrive,
    // m_rightCamera));
    ...
    m_configureController.leftTrigger(OIConstants.kTriggerThreshold)
        .whileTrue(m_superstructure.configureRetractLeftClimber())
        .whileFalse(m_climber.stopLeftClimber());
    m_configureController.rightTrigger(OIConstants.kTriggerThreshold)
        .whileTrue(m_superstructure.configureRetractRightClimber())
        .whileFalse(m_climber.stopRightClimber());
  ...
  public void setTeleopDefaultStates() {
    ...
    m_robotDrive.setHeading(
        DriverStation.getAlliance().toString().equals("Red")
            ? 180 - m_robotDrive.getHeading()
